...
if [[ -f ~/dotfile/.thismachine ]]; then
	source ~/dotfiles/.thismachine
fi
...
function parse_svn_dirty() {
	[[ $(svn st $1 -q 2> /dev/null | wc | awk '{print $1}') -ne 0 ]] && echo '+'
}
...
function parse_svn_total() {
	echo $(parse_svn_branch)$(parse_svn_dirty $PWD)
}
...
# for svn:
PS1="\[\e]2;$PWD\[\a\]\[\e]1;\]$(basename "$(dirname "$PWD")")/\W\[\a\]${BOLD}\$(usernamehost)$(airplane)\[\$RESET\]\[\$GREEN\]\w\[\$BOLD\]\[$WHITE\]\$([[ -n \$(parse_svn_branch) ]] && echo \" in \")\[$PURPLE\]\$(parse_svn_total)\[$WHITE\]\n\$ \[$RESET\]"
